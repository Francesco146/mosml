...
# Install build dependencies (fail fast, print commands, and preserve pipefail)
RUN bash -euxo pipefail -c "apt-get update && \
...
# Install runtime dependencies (fail fast and show commands)
RUN bash -euxo pipefail -c "apt-get update && \
    apt-get install -y --no-install-recommends \
    libc6 \
    libgmp10 \
...
# Default command is interactive REPL. Keep ENTRYPOINT as the executable and
# provide an empty CMD so callers can pass flags/arguments via `docker run`.
ENTRYPOINT ["mosml"]
CMD []
